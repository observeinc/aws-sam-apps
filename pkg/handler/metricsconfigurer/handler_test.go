...
func TestParseResponse(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result, err := h.parseResponse(tc.responseBody, mockRequest)
...
func TestGetDatasource(t *testing.T) {
	...
	req := &Request{
		RequestType:       "Create",
		ResponseURL:       mockServer.URL,
		StackId:           "arn:aws:cloudformation:us-east-1:123456789:stack/MyStack/guid",
		RequestId:         "unique_id",
		ResourceType:      "Custom::MyCustomResource",
		LogicalResourceId: "MyCustomResource",
	}
	...
	result, err := h.getDatasource(req, &token, domainName, mockClient)
	if err != nil {
		t.Fatalf("Error calling getDatasource: %v", err)
	}

	// Call parseResponse with the result
	filters, err := h.parseResponse(result, req)
	if err != nil {
